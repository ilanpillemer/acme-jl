...
func main() {
	...
	for {
		...
		//If log reports that a jl file is being "put", then format it
		if event.Name != "" && event.Op == "put" && strings.HasSuffix(event.Name, ".jl") {
			juliaFormat(event.ID, event.Name)
		}
	}
}

func juliaFormat(id int, name string) {
	...
	// this formats the file in place.  So
	// after running this the file on disk will be reformatted, but not
	// the file in the Acme window

	cmd := fmt.Sprintf("using JuliaFormatter; format(\"%s\")", name)

	out, err := exec.Command("julia", "-e", cmd).CombinedOutput()
	if err != nil {
		log.Printf("%s", out)
		return
	}
